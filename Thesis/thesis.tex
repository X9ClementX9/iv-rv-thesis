...
\usepackage{bibentry}
% \fcitet{key} -> "Author (Year)" + a footnote with the full reference
% \fcitet{key}: inline citation + footnote with full reference.
% The visible citation AND the footnote superscript both link to the
% bibliography entry (not to the footnote text at the bottom of the page).
\newcommand{\fcitet}[1]{%
  \citet{#1}%
  \refstepcounter{footnote}%
  \hyperlink{cite.#1}{\textsuperscript{\thefootnote}}%
...
\nobibliography*    % required by bibentry to inline references in footnotes
...
received substantial empirical support. \fcitet{christensen1998} show that implied
...
volatility models and econometric models such as ARCH and GARCH. \fcitet{poon2003} review a large body of
...
These three findings provide the equity benchmark against which we
evaluate Bitcoin in this thesis. They motivate the use of the standard
predictive regression $RV_{t+h}=\alpha+\beta IV_t+\varepsilon$ and the
joint efficiency test $H_0:\alpha=0,\beta=1$, both presented in
Chapter~\ref{data-and-methods}. If crypto volatility markets are less
mature, we expect a lower slope, a larger intercept, and a stronger
regime dependence than for the S\&P 500.
...
The empirical evidence is more nuanced. Early work by \fcitet{canina1993} questions the idea that implied
...
\fcitet{christensen1998} study the
...
variance risk premium. \fcitet{bollerslev2009} show that the difference between implied and realized
...
volatility forecasting. \fcitet{poon2003}
...
This balanced view shapes our hypotheses H1 and H2. We expect implied
volatility to have significant predictive power (slope $\beta$
different from zero), but also to be biased (slope $\beta$ different
from one, positive intercept). The fact that forecasting performance
depends on the asset class also justifies estimating the predictive
regression separately on Bitcoin and on the S\&P 500, rather than
pooling them.
...
premium is positive on average in equity markets. \fcitet{bollerslev2009} show that the difference
...
than a simple forecasting error. Similarly, \fcitet{carr2009} provide
...
This literature provides two priors for our analysis. First, the
average VRP should be positive in both markets, and larger in markets
where risk aversion or hedging demand is structurally higher. This is
our motivation for H3 (larger VRP in crypto). Second, the time
variation of the VRP, and not only its level, carries economic
content. This motivates the regime-based analysis of the VRP in
Table~\ref{tab:vrp} rather than a single pooled estimate.
...
\fcitet{engle1982} and \fcitet{bollerslev1986} provide early evidence of time-varying volatility
...
used as a measure of market stress. \fcitet{whaley2009} shows that the VIX reflects
...
example, \fcitet{giot2005} finds that implied volatility
...
regime-dependent. \fcitet{hamilton1989} introduces the concept of regime-switching models,
...
The evidence that the IV--RV relationship weakens in crises is what
H4 and H5 try to quantify in our 2021--2025 sample. This sample
includes several stress periods, such as the May 2022 crypto
deleveraging, the March 2023 banking turmoil, and the mid-2024 yen
carry-trade unwind. We do not assume regime dependence: we test for it
by splitting the sample with 90th-percentile thresholds
(Section~\ref{definition-of-market-regimes}) and by re-estimating the
predictive regression in each regime.
...
classes. \fcitet{dyhrberg2016} studies Bitcoin through a GARCH framework
...
subject to strong regime changes. \fcitet{baur2018} revisit the comparison between
...
These features lead to specific predictions for the cross-market
comparison. A retail-heavy investor base and continuous trading should
increase the demand for hedging and speculative positions on options.
This is expected to raise DVOL above the level justified by future
realized volatility, and therefore to produce a larger variance risk
premium in crypto than in equities. This is the basis of H3.

A more concentrated trading venue (mainly Deribit) and thinner
liquidity for some options should add noise to the implied volatility
measure. As a result, we expect a lower predictive slope for Bitcoin
than for the S\&P 500, in line with H2.

Cryptocurrency markets also trade 24/7, so realized volatility may
react faster to news than implied volatility. This can lead to a
compression of the variance risk premium during stress in crypto. In
equities, the opposite is expected: implied volatility tends to react
first, which widens the premium. This contrast is one of the main
empirical predictions we test in
Section~\ref{variance-risk-premium-analysis}.

Finally, a less mature regulatory and informational environment
suggests that the predictive relationship will be less stable in
crypto than in equities. This justifies H4 and motivates the
regime-conditional regressions in Table~\ref{tab:reg_regime}.
...
This is the gap we aim to fill. By applying the standard equity
predictive regression to Bitcoin with DVOL as the implied volatility
input, we provide direct estimates of the IV--RV relationship in
crypto, both globally and by regime, over a multi-year sample. The
use of aggregated indices is a deliberate choice: we lose some
information on the volatility surface, but we gain a clean comparison
with the VIX-based equity literature.
...
from a rolling window of $h$ daily returns. Two consecutive
observations therefore share $h-1$ underlying returns. This creates a
moving-average structure in the residuals, which makes ordinary OLS
standard errors unreliable. To correct for this, we use HAC standard
errors following \fcitet{newey1987}, with a Bartlett kernel and a lag
truncation equal to the horizon ($q = h = 30$ for the main
specification). All $t$-statistics and $p$-values reported in
Chapter~\ref{empirical-results} use these HAC standard errors.
...
\begin{equation}
\mathbf{1}^{\text{stress}}_{SPX,t} = \mathbf{1}\!\left\{ VIX_t > Q_{0.90}(VIX) \right\},
\label{eq:stress_spx}
\end{equation}
...
\begin{equation}
\mathbf{1}^{\text{stress}}_{BTC,t} = \mathbf{1}\!\left\{ RV^{(30)}_{BTC,t} > Q_{0.90}(RV^{(30)}_{BTC}) \right\}.
\label{eq:stress_btc}
\end{equation}
...
\begin{equation}
\mathbf{1}^{\text{stress}}_{\text{global},t} = \mathbf{1}^{\text{stress}}_{SPX,t} \vee \mathbf{1}^{\text{stress}}_{BTC,t}.
\label{eq:stress_global}
\end{equation}

\textbf{Why an asymmetric design.} The two markets are not classified
using the same variable, and this choice is intentional. For equities,
the VIX is the natural option-implied measure of stress. Using
realized volatility instead would mix the dependent variable of the
predictive regression with the regime classifier. For Bitcoin, no
option-based index has enough history to cover the full sample, so
realized volatility is the only available option. We discuss this
asymmetry as a limitation in
Section~\ref{limitations-of-the-study}. Using DVOL for the crypto
regime when it is available gives similar results.
...
Figure~\ref{fig:iv_rv_timeline} shows the difference in volatility
levels between the two markets. Bitcoin volatility moves in a range
about four times wider than the S\&P 500. The gap between implied and
realized volatility is positive on average in both markets. It also
narrows around the BTC stress periods (May 2022 and early 2024) and
widens around the SPX stress periods (in particular March 2023).
Tables~\ref{tab:desc} and~\ref{tab:vrp} report the numbers behind
these patterns.
...
spikes) is longer than what a symmetric benchmark would imply. Both $RV^{(30)}$ and $IV$ are positively skewed in both markets
($+0.78$ and $+0.73$ for Bitcoin, $+1.60$ and $+1.34$ for the S\&P
500). This means that very high volatility episodes occur more often
than a symmetric distribution would suggest. The asymmetry is stronger
in equities, where occasional VIX spikes during stress periods stand
out from otherwise calm periods.

Excess kurtosis goes in the same direction. The S\&P 500 volatility
series have fat tails ($+2.69$ for $RV^{(30)}$, $+2.64$ for the VIX).
Extreme high-volatility observations are therefore more frequent than
under a normal distribution. Bitcoin volatility is much closer to
mesokurtic ($+0.54$ and $+0.02$). The reason is that high volatility
is the norm rather than the exception in crypto, so the separation
between calm and stress periods is less sharp.
...
Figure~\ref{fig:iv_rv_scatter} illustrates the main result of the
predictive regression. In both markets, the OLS slope is clearly
flatter than the 45$^\circ$ line. The gap is much wider for Bitcoin
than for the S\&P 500. Table~\ref{tab:reg_global} reports the
...
Table~\ref{tab:summary_xmarket} summarises the comparison between
the two markets on the main dimensions of the analysis. The structural
interpretation of these differences (market maturity, microstructure,
investor base, and 24/7 trading) is discussed in
...
Two results stand out from this comparison. First, the gap between
the slopes does not only reflect a difference in level. In normal
periods, the S\&P 500 is statistically close to full efficiency
($p=0.89$ for $\beta=1$), while Bitcoin remains clearly below one,
even though the difference in absolute values is moderate ($0.97$ vs
$0.54$). Second, the variance risk premium reacts in opposite
directions to stress in the two markets. This pattern only appears
when both markets are studied together.
...
The empirical results point to three main findings.

First, implied volatility contains useful forward-looking information
in both markets. The slopes in Table~\ref{tab:reg_global} are highly
significant and the $R^2$ exceeds 0.32 in each case. This confirms
that DVOL and the VIX are valuable forecast inputs.

Second, neither slope is consistent with full efficiency. Both are
below one, and the variance risk premium is positive on average. This
is the standard pattern associated with a positive premium for
volatility risk. Implied volatility is therefore better interpreted as
a risk-adjusted expectation than as a pure forecast.

Third, the relationship depends on the market regime, but in a
different way for each market. For the S\&P 500, the slope is close
to one in normal periods ($p=0.89$ for $\beta=1$) and collapses to
$0.32$ in stress, with the $R^2$ falling to $0.05$. For Bitcoin, the
slope is already below one in normal periods, and the drop in stress
is smaller. The crypto-stress sample is also too short ($N=117$) to
identify a significant change with confidence.

The behaviour of the variance risk premium reflects the same
asymmetry. It widens in equity stress and compresses in crypto stress.
These three patterns are the main empirical contribution of the
thesis. The rest of this chapter discusses the economic mechanisms
behind them.
...
such as \fcitet{christensen1998} find that implied volatility has strong predictive power,
...
research. \fcitet{bollerslev2009} show that the variance risk premium is positive on average
...
This thesis asked whether implied volatility efficiently predicts
future realized volatility, and how this relationship varies across
market regimes and between cryptocurrency and equity markets. The
empirical results give three answers.

First, implied volatility is informative but biased. In both Bitcoin
and the S\&P 500, the slope of the predictive regression is highly
significant, with $R^2$ above 0.32. The slope is also significantly
below one, and the variance risk premium is positive on average
($+0.08$ for Bitcoin, $+0.04$ for the S\&P 500). Implied volatility
should therefore be read as a risk-adjusted expectation, not as a pure
forecast.

Second, equity markets are close to efficiency in normal times. The
S\&P 500 normal-regime slope is $0.97$ with an HAC standard error of
$0.20$, and the test $\beta=1$ gives $p=0.89$. In other words, we
cannot reject that the VIX is a fully efficient predictor when markets
are calm. In stress, the slope falls to $0.32$ and $R^2$ drops to
$0.05$. Bitcoin is different: the slope stays below one even in calm
periods ($0.54$), and the crypto-stress sample is too short ($N=117$)
to identify a significant change.

Third, the variance risk premium reacts differently to stress across
the two markets. It widens for the S\&P 500 (the VIX rises faster than
realized volatility) and compresses for Bitcoin (realized volatility
catches up faster than DVOL). This contrast is one of the main
empirical contributions of the thesis. It is consistent with the
market structure arguments developed in Chapter~\ref{discussion}.
...
The main message of this thesis is that DVOL behaves like the VIX, but
in a market that is less efficient. Implied volatility is informative
in both Bitcoin and the S\&P 500. However, only the S\&P 500 in normal
periods is close to being an unbiased predictor. In stress periods,
the predictive relationship weakens in both markets. The variance risk
premium also moves in opposite directions: it widens in equities and
compresses in crypto. For practitioners, this means that volatility
indices should be used as risk-adjusted expectations, not as raw
forecasts. Their reliability is also lower in stress, which is exactly
when accurate forecasts matter most.
